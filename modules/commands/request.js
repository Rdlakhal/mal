...
api.changeNickname(`${(!global.config.BOTNAME) ? "Made by O M A R" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage({body:`「 تمت الموافقه 」`, attachment: fs.createReadStream(__dirname + "/../../kur.mp4")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    if (list.length != 0) return api.sendMessage(`❯ إجمالي عدد حسابات: ${list.length}  \n\n${msg}\n❯  الرد برقم للموافقة`, threadID, (error, info) => {
    ...
    else return api.sendMessage("❯ لا يوجد حاليا أي حسابات في قائمة الانتظار", threadID, messageID);
}
    case "thread":
    case "-t":
    case "t":
